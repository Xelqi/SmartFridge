...
function ShoppingList({ shoppingLists, setShoppingLists }) {
  ...
  const deleteList = async (listId) => {
    try {
      const response = await fetch(`/api/shopping/delete-shopping-list/${listId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const updatedListsResponse = await fetch("/api/shopping/get-all-shopping-lists");
      if (!updatedListsResponse.ok) {
        throw new Error("Failed to fetch updated shopping lists");
      }
      const updatedListsData = await updatedListsResponse.json();
  ...
  return (
    <div className="container mt-2 overflow-y-auto" style={{ height: "calc(100vh - 57px - 65px - 50px)" }}>
          ...
          <div className="card-body">
            <div 
              ...
              <div className="card-title d-flex justify-content-between align-items-baseline fw-normal mb-0">
                ...
                <button className="btn p-0 m-0" id="buttons" onClick={() => deleteList(list._id)}>
